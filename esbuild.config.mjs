...
async function main() {
  ...
  const codiconsDest = join(__dirname, 'dist', 'webview', 'codicons');
  mkdirSync(codiconsDest, { recursive: true });
  cpSync(join(codiconsSrc, 'codicon.css'), join(codiconsDest, 'codicon.css'));
  cpSync(join(codiconsSrc, 'codicon.ttf'), join(codiconsDest, 'codicon.ttf'));
  ...
  if (isWatch) {
    ...
    await Promise.all([extCtx.watch(), sidebarCtx.watch(), graph2dCtx.watch(), addonsCtx.watch()]);
  } else {
