...
- Browser-side webview UI for a domain lives in that domain’s `webview/` folder (e.g. `domains/graph/webview/`); it is bundled separately and runs in the webview, not in the extension host.
