...
import type { AddonLocality } from '../domain/cliMappings';
...
export interface AddonsPanelEnvironment {
  ...
  addOrigin: (source: { kind: string; value: string }) => Promise<void>;
  editOrigin: (originId: string, source: { kind: string; value: string }) => Promise<void>;
