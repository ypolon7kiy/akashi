...
export class AddonsPanel {
    ...
    this.panel.webview.onDidReceiveMessage(
      async (message: { type?: string; payload?: unknown }) => {
        ...
        if (message?.type === AddonsMessageType.AddOrigin) {
          const p = message.payload as { kind?: string; value?: string } | undefined;
          if (p?.kind && p?.value) {
            await this.snapshotEnv?.addOrigin({ kind: p.kind, value: p.value });
        ...
        if (message?.type === AddonsMessageType.EditOrigin) {
          const p = message.payload as
            | { originId?: string; kind?: string; value?: string }
            | undefined;
          if (p?.originId && p?.kind && p?.value) {
            try {
              await this.snapshotEnv?.editOrigin(p.originId, {
                kind: p.kind,
                value: p.value,
              });
              await this.refreshAfterMutation();
        ...
        if (message?.type === AddonsMessageType.InstallPlugin) {
          const p = message.payload as
            | { pluginId?: string; locality?: 'workspace' | 'local' | 'user' }
            | undefined;
          if (p?.pluginId) {
            const locality =
              p.locality === 'user' ? 'user' : p.locality === 'local' ? 'local' : 'workspace';
  ...
  public async pushCatalog(env: AddonsPanelEnvironment): Promise<void> {
    ...
    const installed = payload?.cliAvailable
      ? (payload?.cliInstalledPlugins?.length ?? 0)
      : (payload?.records?.length ?? 0);
    const available = payload?.catalogPlugins?.length ?? 0;
    appendLine(
      `[Akashi][Addons] postMessage catalog installed=${installed} available=${available} cliAvailable=${payload?.cliAvailable ?? false}`
    );
