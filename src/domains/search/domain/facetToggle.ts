...
/**
 * Like {@link toggleEnabledId} but prevents deselecting the last enabled item.
 * Returns the previous state unchanged when the toggle would leave zero items enabled.
 */
export function toggleEnabledIdMinOne(
  prev: ReadonlySet<string> | null,
  fullSortedIds: readonly string[],
  id: string
): Set<string> | null {
  const next = toggleEnabledId(prev, fullSortedIds, id);
  if (next !== null && next.size === 0) {
    return prev === null ? null : new Set(prev);
  }
  return next;
}
