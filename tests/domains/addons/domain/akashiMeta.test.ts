...
describe('parseAkashiMeta', () => {
  ...
  it('preserves installedAt when present', () => {
    const raw = {
      version: 1,
      installed: {
        claude: [
    ...
    const meta = parseAkashiMeta(raw);
    expect(getEntries(meta, 'claude')[0].installedAt).toBe('2026-03-29T00:00:00.000Z');
  });

  it('returns undefined installedAt when absent', () => {
    const raw = {
      version: 1,
      installed: {
        claude: [{ name: 'foo', category: 'skill', originId: 'origin-a', version: '1.0.0' }],
    ...
    const meta = parseAkashiMeta(raw);
    expect(getEntries(meta, 'claude')[0].installedAt).toBeUndefined();
  });

  it('preserves a valid catalogSnapshot', () => {
    // Inner snapshot uses 'plugin' category while outer entry uses 'skill' —
    // intentionally testing that snapshot category is stored independently.
    const snapshot = {
      id: 'foo@origin-a',
      originId: 'origin-a',
      name: 'foo',
      ...
      tags: ['test'],
      keywords: [],
      source: { kind: 'relative', path: './plugins/foo' },
    };
    const raw = {
      version: 1,
      installed: {
    ...
    const meta = parseAkashiMeta(raw);
    const parsed = getEntries(meta, 'claude')[0].catalogSnapshot;
    expect(parsed).toBeDefined();
    expect(parsed!.name).toBe('foo');
    expect(parsed!.category).toBe('plugin');
    expect(parsed!.source).toEqual({ kind: 'relative', path: './plugins/foo' });
    expect(parsed!.tags).toEqual(['test']);
  });

  it('returns undefined catalogSnapshot when malformed (missing id)', () => {
    const raw = {
      version: 1,
      installed: {
        claude: [
          {
            ...
            originId: 'origin-a',
            version: '1.0.0',
            catalogSnapshot: { name: 'foo' }, // missing id
          },
        ],
      },
    };
    const meta = parseAkashiMeta(raw);
    expect(getEntries(meta, 'claude')[0].catalogSnapshot).toBeUndefined();
  });

  it('returns undefined catalogSnapshot when absent', () => {
    const raw = {
      version: 1,
      installed: {
        claude: [{ name: 'foo', category: 'skill', originId: 'origin-a', version: '1.0.0' }],
