...
describe('AddonsService.editOrigin', () => {
  it('updates source without changing the id when source stays the same', async () => {
    ...
    const result = await service.editOrigin('github:acme/skills', source);

    expect(result.id).toBe('github:acme/skills');
    // Label is now auto-derived from source
    expect(result.label).toBe('acme/skills');
  ...
  it('clears old cache when source changes (id changes)', async () => {
    ...
    const result = await service.editOrigin('github:acme/skills', newSource);
  ...
  it('throws when origin id is not found', async () => {
    ...
    await expect(
      service.editOrigin('github:nope/nope', { kind: 'url', url: 'https://x.com' })
  ...
  it('throws when new source collides with another existing origin', async () => {
    ...
    await expect(service.editOrigin('github:acme/skills', sourceB)).rejects.toThrow(
  ...
  it('preserves position in the origins array', async () => {
    ...
    await service.editOrigin('url:https://b.com/market.json', newSource);
    ...
    expect(saved[1].label).toBe('b-new.com'); // auto-derived from URL
...
describe('editOrigin auto-fetch', () => {
  async function editAndFetch(
    ...
    const updated = await service.editOrigin(originId, source);
    if (updated.enabled) {
  ...
  it('fetches catalog after edit when origin is enabled', async () => {
    ...
    await editAndFetch(service, 'github:acme/skills', newSource);
  ...
  it('does not fetch catalog after edit when origin is disabled', async () => {
    ...
    const updated = await service.editOrigin('github:acme/skills', newSource);
