...
import type { JsonMergeOp, WriteFileOp } from '@src/domains/sources/domain/artifactOperation';
import { FixedDocCreator } from '@src/domains/sources/domain/creators/FixedDocCreator';
...
import { ClaudeMcpCreator } from '@src/domains/sources/presets/claude/creators/ClaudeMcpCreator';
import { ClaudeRegisteredHookCreator } from '@src/domains/sources/presets/claude/creators/ClaudeRegisteredHookCreator';
import { CursorMcpCreator } from '@src/domains/sources/presets/cursor/creators/CursorMcpCreator';
import { CursorRegisteredHookCreator } from '@src/domains/sources/presets/cursor/creators/CursorRegisteredHookCreator';
import { CodexConfigTomlCreator } from '@src/domains/sources/presets/codex/creators/CodexConfigTomlCreator';
import { DEFAULT_CLAUDE_HOOK_EVENT } from '@src/domains/sources/presets/claude/creators/ClaudeRegisteredHookCreator';
import { DEFAULT_CURSOR_HOOK_EVENT } from '@src/domains/sources/presets/cursor/creators/CursorRegisteredHookCreator';
  ...
  it('all creators: traversal basename policy (error vs plan + structure)', () => {
    ...
    for (const creator of ARTIFACT_CREATORS) {
      ...
      if (expectsBasenameValidationErrorOnTraversal(creator.id)) {
        if (r.kind !== 'error') {
          failures.push(
            `${creator.id}: expected kind 'error' for traversal input, got '${r.kind}'`
          );
        }
      } else {
        if (r.kind !== 'plan') {
...
describe('Creator matrix: registered hook + MCP plan semantics (planWithProvidedInput)', () => {
  ...
  describe('ClaudeRegisteredHookCreator (registry)', () => {
    ...
    it('claude/hook-config/workspace: unknown lifecycle falls back to default', () => {
      ...
      expect(r.kind).toBe('plan');
      if (r.kind !== 'plan') return;
      expect((r.plan.operations[1] as JsonMergeOp).jsonPath).toBe(
        `hooks.${DEFAULT_CLAUDE_HOOK_EVENT}`
      );
    });
  });
  ...
  describe('CursorRegisteredHookCreator (registry)', () => {
    it.each(['cursor/hook/workspace', 'cursor/hook/user'] as const)(
      ...
      (id) => {
        ...
        expect(entries[0].command).toBe(
          id.endsWith('/workspace') ? `.cursor/hooks/${HOOK_NAME}.sh` : `./hooks/${HOOK_NAME}.sh`
        );
    ...
    it('cursor/hook/workspace: unknown lifecycle falls back to default', () => {
      ...
      if (r.kind !== 'plan') return;
      expect((r.plan.operations[1] as JsonMergeOp).jsonPath).toBe(
        `hooks.${DEFAULT_CURSOR_HOOK_EVENT}`
      );
    });
  });
  ...
  describe('MCP creators (registry)', () => {
    ...
    ] as const)('%s: jsonMerge targets path and mcpServers[%s]', (id, expectedPath, serverKey) => {
      const c = ARTIFACT_CREATORS.find((x) => x.id === id)!;
